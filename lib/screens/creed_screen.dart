...
class _CreedScreenState extends State<CreedScreen> {
  String selectedVersion = "사도신경traditionalKorean"; // 기본 버전 (사도신경)
  Map<String, dynamic> creedData = {};
  ...
  /// 📌 사도신경 데이터 로드
  Future<void> _loadCreedData() async {
    final String jsonString =
        await rootBundle.loadString('assets/data/prayers.json');
    final jsonData = json.decode(jsonString);
    setState(() {
      creedData = jsonData[selectedVersion];
    });
  }

  /// 📌 사도신경 버전 변경
  Future<void> _changeCreedVersion(String newVersion) async {
  ...
  Future<void> _saveSelectedVersion(String version) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('selectedCreedVersion', version); // 키 값 변경
  }
  ...
  Future<void> _loadSelectedVersion() async {
    final prefs = await SharedPreferences.getInstance();
    setState(() {
      selectedVersion = prefs.getString('selectedCreedVersion') ??
          "사도신경traditionalKorean"; // 기본값 설정
    });
  }
          ...
          PopupMenuButton<String>(
            ...
            itemBuilder: (context) => <PopupMenuEntry<String>>[
              PopupMenuItem<String>(
                value: '사도신경traditionalKorean', // value 값 변경
                child: Text(
                  '전통적인 한국어',
                  style: TextStyle(
              ...
              PopupMenuItem<String>(
                value: '사도신경modernKorean', // value 값 변경
                child: Text(
                  '현대적인 한국어',
                  style: TextStyle(
              ...
              PopupMenuItem<String>(
                value: '사도신경traditionalEnglish', // value 값 변경
                child: Text(
                  'Traditional English',
                  style: TextStyle(
                    color: Color(0xFF424874),
              ...
              PopupMenuItem<String>(
                value: '사도신경modernEnglish', // value 값 변경
                child: Text(
                  'Modern English',
                  style: TextStyle(
                    color: Color(0xFF424874),
